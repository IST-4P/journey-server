...
export class BlogService implements OnModuleInit {
  ...
  ): Promise<BlogProto.GetBlogResponse> {
    return lastValueFrom(this.blogService.createBlog(data));
  }

  updateBlog(
    data: BlogProto.UpdateBlogRequest
  ): Promise<BlogProto.GetBlogResponse> {
    return lastValueFrom(this.blogService.updateBlog(data));
  }

  deleteBlog(
    data: BlogProto.DeleteBlogRequest
  ): Promise<BlogProto.DeleteBlogResponse> {
    return lastValueFrom(this.blogService.deleteBlog(data));
  }
}
