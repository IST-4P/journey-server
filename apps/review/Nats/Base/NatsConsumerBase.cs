...
namespace review.Nats.Base
{
    ...
    public abstract class NatsConsumerBase<TEvent> : BackgroundService where TEvent : class
    {
        ...
        {
            Logger.LogInformation("[Review] Starting {ConsumerName}...", ConsumerName);
            ...
            try
            {
                ...
                try
                {
                    ...
                    Logger.LogInformation("[Review] Consumer '{ConsumerName}' created/updated successfully", ConsumerName);
                }
                catch (Exception ex)
                {
                    Logger.LogError(ex, "[Review] Failed to create consumer '{ConsumerName}'", ConsumerName);
                    return;
                }
                ...
                await foreach (var msg in consumer.ConsumeAsync<byte[]>(cancellationToken: stoppingToken))
                {
                    ...
                    {
                        Logger.LogError(ex, "[Review] Error processing message in {ConsumerName}", ConsumerName);
            ...
            {
                Logger.LogError(ex, "[Review] {ConsumerName} failed", ConsumerName);
            }
        }
    }
}
