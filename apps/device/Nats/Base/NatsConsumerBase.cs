...
namespace device.Nats.Base
{
    ...
    public abstract class NatsConsumerBase<TEvent> : BackgroundService where TEvent : class
    {
        ...
        {
            Logger.LogInformation("[Device] Starting {ConsumerName}...", ConsumerName);
            ...
            try
            {
                ...
                try
                {
                    ...
                    Logger.LogInformation("[Device] Consumer '{ConsumerName}' created/updated successfully", ConsumerName);
                }
                catch (Exception ex)
                {
                    Logger.LogError(ex, "[Device] Failed to create consumer '{ConsumerName}'", ConsumerName);
                    return;
                }
                ...
                await foreach (var msg in consumer.ConsumeAsync<byte[]>(cancellationToken: stoppingToken))
                {
                    ...
                    {
                        Logger.LogError(ex, "[Device] Error processing message in {ConsumerName}", ConsumerName);
            ...
            {
                Logger.LogError(ex, "[Device] {ConsumerName} failed", ConsumerName);
            }
        }
    }
}
