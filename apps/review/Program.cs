...
// Add NATS
var natsUrl = Environment.GetEnvironmentVariable("NATS_URL") ?? "nats://localhost:4212";
...
// gRPC clients
var deviceServiceUrl = Environment.GetEnvironmentVariable("DEVICE_GRPC_SERVICE_URL_NET") ?? "http://localhost:500";
builder.Services.AddGrpcClient<Device.DeviceService.DeviceServiceClient>(o =>
{
    o.Address = new Uri(deviceServiceUrl);
});
var rentalServiceUrl = Environment.GetEnvironmentVariable("RENTAL_GRPC_SERVICE_URL_NET") ?? "http://localhost:501";
builder.Services.AddGrpcClient<Rental.RentalService.RentalServiceClient>(o =>
{
    o.Address = new Uri(rentalServiceUrl);
});
var vehicleServiceUrl = Environment.GetEnvironmentVariable("VEHICLE_GRPC_SERVICE_URL_NET") ?? "http://localhost:502";
builder.Services.AddGrpcClient<Vehicle.VehicleService.VehicleServiceClient>(o =>
{
    o.Address = new Uri(vehicleServiceUrl);
});

var bookingServiceUrl = Environment.GetEnvironmentVariable("BOOKING_GRPC_SERVICE_URL_NET") ?? "http://localhost:503";
...
var grpcUrl = Environment.GetEnvironmentVariable("REVIEW_GRPC_SERVICE_URL_NET") ?? "0.0.0.0:5010";
